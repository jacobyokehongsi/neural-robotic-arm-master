...
class RestrictedPickAndPlace(Task):
    def __init__(
        ...
        self.possible_objects = [np.array(
            [0.335, -0.19, -0.07+self.object_size/2]), np.array([0.537, -0.028, self.object_size / 2])]
    ...
    def reset(self) -> None:
        ...
        self.sim.set_base_pose(
            "object", self.object_position, np.array([0.0, 0.0, 0.0, 1.0]))
...
class RestrictedPandaPickAndPlaceEnv(RobotTaskEnv):
    ...
    def __init__(self, render: bool = False, reward_type: str = "sparse", control_type: str = "ee") -> None:
        ...
        robot = Panda(sim, block_gripper=False, base_position=np.array(
            [0.0, 0.0, 0.0]), control_type=control_type)
